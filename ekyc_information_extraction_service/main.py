...
async def extract_information_from_ocr(ocr_input: OCRInput = Body(...)):
    ...
    if ocr_input.use_gemini_fallback and (not key_fields_present_regex or general_message) and IS_GEMINI_CONFIGURED :
        ...
        if gemini_result and not any(k.startswith("error_gemini") for k in gemini_result):
            ...
            if general_message and not key_fields_present_regex:
                 extracted_data_regex["message"] = general_message + " Đã thử fallback qua Gemini."
            elif general_message:
                 extracted_data_regex["message"] = general_message
            ...
            for field_key in fields_to_request_from_gemini:
                ...
                should_override_with_gemini = (extracted_data_regex.get(field_key) is None and gemini_value is not None) or \
                                              (general_message and gemini_value is not None)

                if should_override_with_gemini:
            ...
            extracted_data_regex["message"] = general_message + " Không thể fallback qua Gemini hoặc Gemini gặp lỗi."
    ...
    if general_message and not extracted_data_regex.get("message"):
        extracted_data_regex["message"] = general_message
